...
geneInformation *annotatedGene = new geneInformation[100000]; //no more than 100000 gene can be present in the annotation file
...
geneInformation gff3Analyzer::getGeneInformation(void) //extract blocks of data from the annotation file relative to each gene
{
	...
	for((a=line[1].attribute.find("Name=")+5); a<line[1].attribute.length(); a++)
	{
