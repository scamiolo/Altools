...
double GC_content[1000], AT_content[1000],coverageATAreas[1000],coverageGCAreas[1000];
double nucleotideCounter[1000][100], referenceNucleotideCounter[1000][100];
double transitionType[1000], transversionType[1000], homoSNP[1000], heteroSNP[1000], numIndel[1000];
double insertedBases[1000], deletedBases[1000];
...
void referenceCountSNPNucleotides(char nucleotide, int num);
...
int main(int argc, char** argv) {
	...
	strcpy(varscanCommand,"java -jar VarScan.jar pileup2cns ");
    ...
    cout << "\n\nPileup folder was successfully created.\n Thanks for using Altools.\n" << endl <<flush;
    cout.flush();
	return 0;
}
...
void elaborateIndelStatistics(int num)
{
	...
	{
		if( (firstAlleleIndel.at(0)=='+' && secondAlleleIndel.at(0) == '+') || (firstAlleleIndel.at(0)=='*' && secondAlleleIndel.at(0) == '+') || (firstAlleleIndel.at(0)=='+' && secondAlleleIndel.at(0) == '*') )
		{
			numInsertions[num]++;
			if (firstAlleleIndel.length()>1) insertedBases[num] += firstAlleleIndel.length();
			else insertedBases[num] += secondAlleleIndel.length();
		}

		if( (firstAlleleIndel.at(0)=='-' && secondAlleleIndel.at(0) == '-') || (firstAlleleIndel.at(0)=='*' && secondAlleleIndel.at(0) == '-') || (firstAlleleIndel.at(0)=='-' && secondAlleleIndel.at(0) == '*'))
		{
			numDeletions[num]++;
			if (firstAlleleIndel.length()>1) deletedBases[num] += firstAlleleIndel.length();
			else deletedBases[num] += secondAlleleIndel.length();
		}
		...
		if( (firstAlleleIndel.at(0)=='+' && secondAlleleIndel.at(0) == '-') || (firstAlleleIndel.at(0)=='-' && secondAlleleIndel.at(0) == '+') )
		{
			numInsertions[num]++;
			numDeletions[num]++;
			if (firstAlleleIndel.at(0)=='+') insertedBases[num] += firstAlleleIndel.length();
			else deletedBases[num] += firstAlleleIndel.length();

			if (secondAlleleIndel.at(0)=='+') insertedBases[num] += secondAlleleIndel.length();
			else deletedBases[num] += secondAlleleIndel.length();
...
void referenceCountSNPNucleotides(char nucleotide, int num)
{
	for(a=0;a<15;a++) if(nucleotideName[a] == nucleotide) referenceNucleotideCounter[num][a]++;
		...
		if(readLine.consensusBase.at(0) != '+' && readLine.consensusBase.at(0) != '-' && readLine.consensusBase.at(0) != '*')
			{	//Call snp or reference
				...
				if( (readLine.referenceBase.at(0)!=readLine.consensusBase.at(0)) && readLine.pvalue <= snpPValue)
				{
					...
					referenceCountSNPNucleotides(readLine.referenceBase.at(0),chromosomeAnalyzed);
		...
		for(i=0;i<100;i++)
			{
				...
				referenceNucleotideCounter[a][i]=0;
			}
	}
...
void writeUpOuputFile(void)
{
	...
	outputFile<<endl<<"Total Number of SNPs\t";
	for (a=1;a<=chromosomeAnalyzed;a++) outputFile << numSNP[a] << "\t";
	...
	outputFile<<endl<<"Total Number of insertions\t";
	for (a=1;a<=chromosomeAnalyzed;a++) outputFile << numInsertions[a] << "\t";
	...
	outputFile<<endl<<"Total Number of deletions\t";
	for (a=1;a<=chromosomeAnalyzed;a++) outputFile << numDeletions[a] << "\t";
	...
	outputFile<<endl<<"Total Number of Inserted bases\t";
	for (a=1;a<=chromosomeAnalyzed;a++) outputFile << insertedBases[a] << "\t";
	outputFile<<endl<<"Total Number of Deleted bases\t";
	for (a=1;a<=chromosomeAnalyzed;a++) outputFile << deletedBases[a] << "\t";
	...
	for(i=0;i<15;i++)
	{
		outputFile <<nucleotideName[i] << " content (num. SNP) in Analyzed Genome\t";
	...
	for(i=0;i<15;i++)
	{
		outputFile <<nucleotideName[i] << " content (num. SNP) in Reference Genome \t";
		for (a=1;a<=chromosomeAnalyzed;a++) outputFile << referenceNucleotideCounter[a][i]/numSNP[a] << "\t";
		outputFile << "\n";
	}
